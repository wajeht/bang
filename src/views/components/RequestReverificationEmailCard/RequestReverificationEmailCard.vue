...
async function requestReverificationEmail(): Promise<void> {
	try {
		states.loading = true;

		const { error, loading, alert, ...inputs } = states;

		await axios.post('/api/v1/auth/request-reverification-email', inputs);

		states.alert = {
			type: 'success',
			message:
				'If you have an account, you will receive an email with instructions on how to reverify email in a few minutes.',
			icon: true,
		};
	...
	<div class="w-full max-w-[400px] flex flex-col gap-6">
		...
		<div class="card bg-base-100 shadow-xl gap-10">
			...
			<div class="card-body gap-6">
				<!-- title -->
				<h2 class="card-title">Request Reverification Email</h2>
				...
				<Button
					...
					@click="requestReverificationEmail"
				/>
			</div>
		</div>
	</div>
